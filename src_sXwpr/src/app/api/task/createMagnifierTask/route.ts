// 发布放大镜任务API路由
// 这是一个Next.js API路由，用于处理发布放大镜任务的HTTP请求
...
// 导入发布放大镜任务的API处理函数
import { handleCreateMagnifierTask } from '../../../../api/handlers/task/createMagnifierTaskHandler';

/**
 * 处理POST请求 - 发布放大镜任务
 * @returns 返回发布放大镜任务的结果
 */
export async function POST(request: Request): Promise<NextResponse> {
  // 直接调用API处理函数，将请求转发给它处理
  // 处理函数会返回标准化的NextResponse响应
  return handleCreateMagnifierTask(request);
}
