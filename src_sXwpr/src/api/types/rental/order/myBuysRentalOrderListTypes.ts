...
// 订单列表数据接口
export interface MyBuysRentalOrderListData {
  list: RentalOrderItem[];
  pagination: Pagination;
}

// API响应数据接口
export interface MyBuysRentalOrderListResponseData {
  code: number;
  message: string;
  data: MyBuysRentalOrderListData;
  timestamp: number;
}
...
export interface MyBuysRentalOrderListResponse {
  success: boolean;
  message: string;
  data: MyBuysRentalOrderListData | null;
}
