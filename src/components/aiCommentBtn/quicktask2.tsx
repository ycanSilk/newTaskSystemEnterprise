...
export default function MiddleCommentGenerator({
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        const response = await fetch('/api/generate-comment', {
          ...
          body: JSON.stringify({
            draft: finalDraft || `请生成一条关于${randomIndustry}的评论`,
  ...
  const generateBatchComments = useCallback(async (
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        // 准备草稿评论，第二条评论（下评评论）直接包含@用户标识
        const finalDrafts = drafts.map((draft, index) => {
          if (index === 1 && atUser) {
            return `@${atUser}`;
          }
          return draft.trim();
        ...
        const response = await fetch('/api/generate-comment', {
          ...
          body: JSON.stringify({
            drafts: finalDrafts.map(d => d || `请生成一条关于${randomIndustry}的评论`),
            industry: randomIndustry,
            sessionId
          }),
        });
  ...
  const finalComments = uniqueResults.map((comment, i) => {
    ...
    // 第二条评论：下评评论，必须添加@用户标识
      ...
      console.log('[Middle Comment Generator] 处理后的下评评论:', processedComment);
  ...
  while (filteredComments.length < commentCount) {
    ...
    if (isUnique) {
      let processedComment = processText(randomPrompt);
      processedComment = applyTextReplacements(processedComment);
      
      // 如果是第二条评论（下评评论），必须添加@用户标识
      if (atUser && filteredComments.length === 1) {
        // 提取实际评论内容（排除@用户标识）
        let commentWithoutAt = processedComment.replace(new RegExp(`@${atUser}`, 'g'), '').trim();
        // 对于带有@用户标识的评论，限制实际内容在7字以内（不包含@用户标识）
        const lastCommentMaxWords = 7;
        ...
        // 插入@用户标识
        processedComment = insertAtUser(trimmedContent, atUser);
        console.log('[Middle Comment Generator] 处理后的补充下评评论:', processedComment);
      }
      // 如果是第一条评论（中评评论），添加name参数
      else if (name && filteredComments.length === 0) {
        if (!processedComment.includes(name)) {
          processedComment = `${name} ${processedComment}`;
        }
      }
