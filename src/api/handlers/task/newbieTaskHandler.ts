...
import { PUBLISH_NEWBIE_TASK_ENDPOINT } from '../../endpoints/task';
import { ApiError, handleApiError, createErrorResponse } from '../../client/errorHandler';
import type { NewBbieTaskRequest, NewBbieTaskResponse, ApiResponse } from '../../types/task/newbieTaskTypes';
 ...
 */
export async function handleNewBbieTask(request: Request): Promise<NextResponse> {
  try {
    // 解析请求体，获取发布任务所需的数据
    const requestData: NewBbieTaskRequest = await request.json();
    
    // 使用apiClient发送POST请求，不手动处理Token，依赖API客户端内置的Token处理
    const response = await apiClient.post<NewBbieTaskResponse>(
      PUBLISH_NEWBIE_TASK_ENDPOINT,
      requestData
    );
