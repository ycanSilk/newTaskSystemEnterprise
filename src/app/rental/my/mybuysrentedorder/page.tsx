            ...
            <div key={order.id} className="bg-white shadow-sm mb-2 p-2 ">
              <Link href={`/rental/my/mysellerrentalorder/detail/${order.id}`}>
                <div className="flex space-x-4 ">
                  {/* 左侧图片 */}
                  <div className="w-1/3 ">
                    <div className="bg-gray-100 overflow-hidden w-full max-h-[120px]">
                  ...
                  <div className="w-2/3 overflow-hidden">
                    ...
                    <p className="text-sm text-gray-500 mb-1 line-clamp-1 space-x-1">
                      ...
                      <span className='text-red-600 text-lg'>￥{order.total_amount_yuan}</span>
                    </p>
                    {/* 标签横排 */}
                    <div className="flex flex-wrap gap-1">
                      {(() => {
                        const tags = [];
                        const buyerInfo = order.seller_info_json || {};
                        
                        // 账号要求标签
                        // 账号要求标签
                        if (buyerInfo.basic_information === 'true') tags.push('修改基本信息');
                        if (buyerInfo.deblocking === 'true') tags.push('账号解禁');
                        if (buyerInfo.identity_verification === 'true') tags.push('实名认证');
                        if (buyerInfo.post_douyin === 'true') tags.push('发布抖音视频');
                        if (buyerInfo.order_requirements === 'true') tags.push('其他');
                        
                        // 登录方式标签
                        if (buyerInfo.scan_code === '1') tags.push('扫码登录');
                        if (buyerInfo.phone_message === '1') tags.push('短信验证');
                        if (buyerInfo.requested_all === '1') tags.push('按租赁方要求');
            ...
            {loading ? (
              ...
              <div key={order.id} className="bg-white p-4 shadow-sm">
                <Link href={`/rental/my/mysellerrentalorder/detail/${order.id}`}>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {(() => {
                      ...
                      // 账号要求标签
                        ...
                        // 登录方式标签
                        if (buyerInfo.scan_code === 'true') tags.push('扫码登录');
                        if (buyerInfo.phone_message === 'true') tags.push('短信验证');
                        if (buyerInfo.requested_all === 'true') tags.push('按租赁方要求');
                  ...
                  <div className="flex gap-2 mt-1 justify-end">
                    <button 
                      className="text-sm border border-gray-400 rounded-sm px-3 py-1"
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        handleContactService(order.id);
