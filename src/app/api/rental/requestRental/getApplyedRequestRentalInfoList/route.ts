...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const status = url.searchParams.get('status') ? parseInt(url.searchParams.get('status') || '0') : undefined;
    const my = url.searchParams.get('my') ? parseInt(url.searchParams.get('my') || '0') : undefined;
