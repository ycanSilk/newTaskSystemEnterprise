...
function validateRegisterData(data: RegisterRequest): { isValid: boolean; error?: string } {
  ...
  // 企业名称长度验证
  if (data.organization_name.length < 2 || data.organization_name.length > 100) {
    return { isValid: false, error: '企业名称长度应在2-100个字符之间' };
  }

  // 企业负责人验证（必填）
  if (!data.organization_leader || data.organization_leader.trim() === '') {
    return { isValid: false, error: '企业负责人不能为空' };
  }

  // 企业负责人长度验证
  if (data.organization_leader.length < 2 || data.organization_leader.length > 50) {
    return { isValid: false, error: '企业负责人姓名长度应在2-50个字符之间' };
  }

  // 企业负责人格式验证（中文、英文、空格）
  const organization_leaderRegex = /^[\u4e00-\u9fa5a-zA-Z\s]+$/;
  if (!organization_leaderRegex.test(data.organization_leader.trim())) {
  ...
  try {
    ...
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        organization_leader: data.organization_leader
      }),
    });
